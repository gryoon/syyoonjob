  ...
  return (
    <div>
      <h2 id="job-heading" data-cy="JobHeading">
        <Translate contentKey="syyoonjobApp.job.home.title">Jobs</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="syyoonjobApp.job.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/job/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="syyoonjobApp.job.home.createLabel">Create new Job</Translate>
          </Link>
        </div>
      </h2>
        ...
        {jobList && jobList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="syyoonjobApp.job.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('jobTitle')}>
                  <Translate contentKey="syyoonjobApp.job.jobTitle">Job Title</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('jobTitle')} />
                </th>
                <th className="hand" onClick={sort('minSalary')}>
                  <Translate contentKey="syyoonjobApp.job.minSalary">Min Salary</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('minSalary')} />
                </th>
                <th className="hand" onClick={sort('maxSalary')}>
                  <Translate contentKey="syyoonjobApp.job.maxSalary">Max Salary</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('maxSalary')} />
                </th>
                <th>
                  <Translate contentKey="syyoonjobApp.job.employee">Employee</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="syyoonjobApp.job.home.notFound">No Jobs found</Translate>
            </div>
          )
        )}
      </div>
